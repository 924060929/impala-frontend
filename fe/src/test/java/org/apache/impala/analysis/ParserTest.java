...
public class ParserTest extends FrontendTestBase {
  ...
  public void TestPlanHints() {
    ...
    for (String[] hintStyle: hintStyles) {
      ...
      // Test hints in a multi-way join (flipped prefix/suffix -> bad hint startPosition/ends).
  ...
  public void TestFromClause() {
    ...
    ParsesOk("select * from src src1 " +
        ...
        "order by src1.key, src1.value, src2.key, src2.objectValue, src3.key, src3.objectValue");
  ...
  public void TestWhereClause() {
    ...
    ParsesOk("select a, b from t where case a when b then true else false endPosition");
  ...
  public void TestHaving() {
    ...
    ParsesOk("select count(c) from test group by a " +
        "having case a when b then true else false endPosition");
    // Arbitrary non-predicate exprs parse ok but are semantically incorrect.
  ...
  public void TestIdentQuoting() {
    ...
    // Whitespace is trimmed from the beginning and endPosition of an identifier.
  ...
  public void TestCaseExprs() {
    // Test regular exps.
    ParsesOk("select case a when '5' then x when '6' then y else z endPosition from t");
    ParsesOk("select case when 'a' then x when false then y else z endPosition from t");
    // Test predicates in case, when, then, and else exprs.
    ParsesOk("select case when a > 2 then x when false then false else true endPosition from t");
    ParsesOk("select case false when a > 2 then x when '6' then false else true endPosition " +
        "from t");
    // Test NULLs;
    ParsesOk("select case NULL when NULL then NULL when NULL then NULL else NULL endPosition " +
        "from t");
    ParsesOk("select case when NULL then NULL when NULL then NULL else NULL endPosition from t");
    // Missing endPosition.
    ParserError("select case a when true then x when false then y else z from t");
    // Missing else after first when.
    ParserError("select case a when true when false then y else z endPosition from t");
    // Incorrectly placed comma.
    ParserError("select case a when true, false then y else z endPosition from t");
  }
  ...
  public void TestCreateAggregate() {
    ...
    // Optional args must be at the endPosition
    ParserError("CREATE UNKNOWN FUNCTION " + "Foo() RETURNS INT" + loc);
  ...
  public void TestGetErrorMsg() {
    ...
    // Long line: error close to the startPosition
    ...
    // Long line: error close to the endPosition
