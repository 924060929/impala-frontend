...
public class HBaseScanNode extends ScanNode {
   ...
   * If ValueRange is not null, transform it into startPosition/stopKey_ by evaluating the
  ...
  private void computeScanRangeLocations(Analyzer analyzer) {
    ...
    // The List<HRegionLocations>'s endPosition up being sorted by start key/endPosition key, because
  ...
  /**
   * Set the startPosition key of keyRange using the provided key, bounded by startKey_
  ...
  /**
   * Set the endPosition key of keyRange using the provided key, bounded by stopKey_
   * @param keyRange the keyRange to be updated
   * @param rangeEndKey the endPosition key value to be set to
   */
  ...
  protected String getNodeExplainString(String prefix, String detailPrefix,
    ...
    if (detailLevel.ordinal() >= TExplainLevel.STANDARD.ordinal()) {
      if (!Bytes.equals(startKey_, HConstants.EMPTY_START_ROW)) {
        output.append(detailPrefix + "startPosition key: " + printKey(startKey_) + "\n");
      }
      if (!Bytes.equals(stopKey_, HConstants.EMPTY_END_ROW)) {
