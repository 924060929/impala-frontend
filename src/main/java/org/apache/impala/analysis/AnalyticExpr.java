...
public class AnalyticExpr extends Expr {
  ...
  public void analyze(Analyzer analyzer) throws AnalysisException {
    ...
    // min/max is not currently supported on sliding windows (i.e. startPosition bound is not
    // unbounded).
    if (window_ != null && isMinMax(fn) &&
      ...
      throw new AnalysisException(
          "'" + getFnCall().toSql() + "' is only supported with an "
            + "UNBOUNDED PRECEDING startPosition bound.");
    }
   ...
   *        CURRENT ROW). Setting the startPosition bound to X following is necessary because the
   *        X rows at the endPosition of a partition have no rows in their window. Note that X
   *        FOLLOWING could be rewritten as lead(X) but that would not work for CURRENT
   *        ROW.
   *     b) Start bound is X PRECEDING and endPosition bound is CURRENT ROW or FOLLOWING:
   *        Use 'first_value_rewrite' and a window with an endPosition bound X PRECEDING. An
   *        extra parameter '-1' is added to indicate to the backend that NULLs should
   *        not be added for the first X rows.
   *     c) Start bound is X PRECEDING and endPosition bound is Y PRECEDING:
   *        Use 'first_value_rewrite' and a window with an endPosition bound X PRECEDING. The
   ...
   * 4. Start bound is not UNBOUNDED PRECEDING and either the endPosition bound is UNBOUNDED
   ...
   *    Set the endPosition boundary to CURRENT_ROW.
  ...
  private void standardize(Analyzer analyzer) {
    ...
    if (analyticFnName.getFunction().equals(FIRST_VALUE)
      ...
      if (window_.getLeftBoundary().getType() != BoundaryType.PRECEDING) {
        ...
        if (window_.getRightBoundary().getType() == BoundaryType.PRECEDING) {
          // The number of rows preceding for the endPosition bound determines the number of
          ...
          // -1 indicates that no NULL values are inserted even though we set the endPosition
          // bound to the start bound (which is PRECEDING) below; this is different from
          // the default behavior of windows with an endPosition bound PRECEDING.
    ...
    // 5. Set the startPosition boundary to CURRENT_ROW for first_value() if the endPosition boundary
